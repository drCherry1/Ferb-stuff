...
def add_subject():
    ...
    st.session_state.subjects.append(f"Subject {number}")
...
def calculate_grades():
    ...
    for i, subject in enumerate(st.session_state.subjects):
        ...
        grade, gpa = get_grade(percentage)
...
st.write("Create your own multiple-choice quiz!")
...
def remove_quiz_question():
    ...
    if len(st.session_state.quiz_questions) > 1:
        ...
        st.warning("You must have at least 1 question.")
...
def next_question():
    ...
    correct = st.session_state.quiz_questions[current]["correct"]

    if selected == correct:
        st.session_state.quiz_score += 1

    if current < len(st.session_state.quiz_questions) - 1:
...
if not st.session_state.quiz_started:
    ...
    if st.button(
        ...
        # Check that every question is filled
        all_filled = True
...
# ==========================================
    ...
    if not st.session_state.quiz_finished:
        ...
        current = st.session_state.quiz_current
        quiz = st.session_state.quiz_questions[current]

        st.subheader(
            f"Question {current + 1} "
            f"of {len(st.session_state.quiz_questions)}"
        )
        ...
        # Convert displayed answer back to A/B/C/D
        answer_map = {
            f"A. {quiz['A']}": "A",
            f"B. {quiz['B']}": "B",
            f"C. {quiz['C']}": "C",
